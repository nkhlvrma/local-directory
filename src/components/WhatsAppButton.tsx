...
export function WhatsAppButton({ listingId, className, compact, iconOnly }: Props) {
  return (
    <a
      ...
      className={`inline-flex h-11 min-h-11 items-center justify-center gap-2 whitespace-nowrap rounded-full bg-[#25D366] text-white font-medium shadow-sm hover:brightness-95 active:scale-[0.98] transition-transform duration-150 ease-out ${
        iconOnly ? "w-11 px-0" : "px-5"
      } ${className ?? ""}`}
    >
      <svg
        aria-hidden="true"
